...
function IconBtn({ item, fallbackColor, fallbackBg, iconSize, errorColor, whiteColor, fontXxs, fontBold, radiusMd, btnSize }) {
  ...
  return (
    <TouchableOpacity onPress={item.onPress} onPressIn={onIn} onPressOut={onOut} activeOpacity={1} disabled={item.disabled} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
      <Animated.View style={[styles.iconWrapper, { transform: [{ scale: scaleAnim }], opacity: item.disabled ? 0.4 : 1 }]}>
        <View style={[styles.iconCircle, { backgroundColor: item.iconBg ?? fallbackBg, width: btnSize, height: btnSize, borderRadius: radiusMd }]}>
          <Ionicons name={item.iconName} size={item.iconSize ?? iconSize} color={item.iconColor ?? fallbackColor} />
        </View>
...
export default function CommonHeader({
  title, subtitle,
  showBack = false, leftIconName = 'arrow-back', leftIconSize, leftIconColor,
  onBackPress, leftActions = [], leftComponent,
  rightActions = [], rightComponent,
  centerComponent, centerTitle = true,
  variant = 'primary', backgroundColor, borderBottom = true, shadow = true,
  transparent = false, animated = true,
}) {
  ...
  const resolvedBg = isTransparent ? 'transparent'
    : backgroundColor ?? (variant === 'gold' ? COLORS.primaryDark : variant === 'white' ? COLORS.white : COLORS.primary);
  ...
  const backItem = { iconName: leftIconName, iconSize: leftIconSize, iconColor: leftIconColor, onPress: handleBack };
  ...
  return (
    <>
      <StatusBar barStyle={onDark ? 'light-content' : 'dark-content'} backgroundColor={isTransparent ? 'transparent' : resolvedBg} translucent={isTransparent} />
      <SafeAreaView edges={['top']} style={{ backgroundColor: isTransparent ? 'transparent' : resolvedBg }}>
        {variant === 'gold' && (
        ...
        <Animated.View style={[{
          ...
          minHeight: verticalScale(58), paddingHorizontal: SIZES.padding.md,
          paddingVertical: SIZES.padding.xs, overflow: 'hidden',
          backgroundColor: resolvedBg,
        },
          borderBottom && { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: onDark ? COLORS.whiteOpacity10 : COLORS.border },
            ...
            {leftComponent ? leftComponent : leftItems.length > 0 ? (
              <View style={styles.iconRow}>
                {leftItems.map((item, i) => (
                  <IconBtn key={i} item={item} fallbackColor={iconFallbackColor} fallbackBg={iconFallbackBg} iconSize={iconSize} errorColor={COLORS.error} whiteColor={COLORS.white} fontXxs={SIZES.font.xxs} fontBold={FONTS.family.bold} radiusMd={radiusMd} btnSize={btnSize} />
                ))}
              </View>
          ...
          <View style={[{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: SIZES.padding.xs }, !centerTitle && { alignItems: 'flex-start' }]}>
            {centerComponent ? centerComponent : (
              <>
                {!!title && <Text numberOfLines={1} style={[FONTS.h5, styles.centerText, { color: titleColor }]}>{title}</Text>}
                {!!subtitle && <Text numberOfLines={1} style={[FONTS.caption, styles.centerText, { color: subColor, marginTop: verticalScale(1) }]}>{subtitle}</Text>}
...
const styles = StyleSheet.create({
  ...
  badgeDot:     { position: 'absolute', top: 2, right: 2, minWidth: 15, height: 15, borderRadius: 999, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 3, borderWidth: 1.5 },
  badgeDotText: { letterSpacing: 0.2 },
