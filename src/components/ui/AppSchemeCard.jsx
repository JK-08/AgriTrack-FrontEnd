...
function SchemeImage({ image, height, gradientColors, children }) {
  ...
  return (
    <View style={{ height, overflow: 'hidden' }}>
      {showImage ? (
        <Image source={src} style={StyleSheet.absoluteFillObject} resizeMode="cover" onError={() => setImgError(true)} />
      ) : (
...
function StatBlock({ value, label, valueColor, bgColor }) {
  return (
...
function ProgressBar({ value, color }) {
  ...
  useEffect(() => {
    Animated.timing(anim, { toValue: Math.min(100, Math.max(0, value)), duration: 600, useNativeDriver: false }).start();
  }, [value]);
...
function WishlistBtn({ active, onToggle }) {
  ...
  return (
    <TouchableOpacity onPress={handlePress} style={styles.wishlistBtn} activeOpacity={0.8}>
      <Animated.View style={{ transform: [{ scale }] }}>
        <Ionicons name={active ? 'heart' : 'heart-outline'} size={18} color={active ? '#DC2626' : '#9CA3AF'} />
      </Animated.View>
    </TouchableOpacity>
  );
}

function ExpiredOverlay({ onRedeem, id }) {
...
function DefaultCard({ scheme, loading, wishlisted, onPress, onBuy, onDetails, onRedeem, onWishlistToggle }) {
  ...
  return (
    <Pressable
      style={[styles.card, { backgroundColor: COLORS.card }, scheme.isFeatured && { borderWidth: 2, borderColor: COLORS.primary }]}
      onPress={() => !isExpired && onPress?.(scheme.id)}
      android_ripple={{ color: COLORS.orangeOpacity10 }}
    >
      {scheme.isFeatured && (
        <View style={[styles.featuredBadge, { backgroundColor: COLORS.primary }]}>
          <Text style={[styles.featuredBadgeText, { fontFamily: FONTS.family.bold, fontSize: SIZES.font.xxs, color: COLORS.white }]}>MOST POPULAR</Text>
        </View>
      )}
      <SchemeImage image={scheme.image} height={140} gradientColors={[COLORS.primaryPale, COLORS.secondaryLighter]}>
        <View style={StyleSheet.absoluteFillObject} pointerEvents="box-none">
          <View style={{ padding: 10, flexDirection: 'row', justifyContent: 'space-between' }}>
            <View style={[styles.badge, { backgroundColor: statusCfg.bg }]}>
              <Text style={[styles.badgeText, { fontFamily: FONTS.family.bold, fontSize: SIZES.font.xxs, color: statusCfg.color }]}>{statusCfg.label}</Text>
            </View>
            <WishlistBtn active={isWishlisted} onToggle={toggleWish} />
          </View>
          <View style={{ position: 'absolute', bottom: 12, left: 14 }}>
            <Text style={{ fontFamily: FONTS.family.bold, fontSize: SIZES.font.xxs, color: COLORS.primaryDark, letterSpacing: 0.5 }}>{scheme.category.toUpperCase()}</Text>
            <Text style={{ fontFamily: FONTS.family.extraBold, fontSize: SIZES.font.xl, color: COLORS.textPrimary, lineHeight: SIZES.font.xl * 1.2 }}>{scheme.name}</Text>
          </View>
        </View>
      ...
      <View style={styles.body}>
        <View style={styles.metaRow}>
          <View style={[styles.categoryPill, { backgroundColor: COLORS.primaryPale }]}>
            <Text style={[styles.categoryText, { fontFamily: FONTS.family.bold, fontSize: SIZES.font.xxs, color: COLORS.primaryDark }]}>{scheme.category}</Text>
          </View>
          {scheme.rating != null && (
            <View style={styles.ratingRow}>
              <Ionicons name="star" size={12} color={COLORS.secondary} />
              <Text style={[styles.ratingText, { fontFamily: FONTS.family.semiBold, fontSize: SIZES.font.xxs, color: COLORS.secondaryDark }]}> {scheme.rating.toFixed(1)}</Text>
              {scheme.reviewCount != null && <Text style={{ fontFamily: FONTS.family.regular, fontSize: SIZES.font.xxs, color: COLORS.textTertiary }}> ({scheme.reviewCount.toLocaleString('en-IN')})</Text>}
            </View>
          )}
        </View>
        <Text style={[styles.schemeName, { fontFamily: FONTS.family.bold, fontSize: SIZES.font.lg, color: COLORS.textPrimary }]}>{scheme.name}</Text>
        <Text style={[styles.schemeDesc, { fontFamily: FONTS.family.regular, fontSize: SIZES.font.sm, color: COLORS.textSecondary }]}>{scheme.description}</Text>
        ...
        {scheme.progress != null && (
          <View style={{ marginBottom: 12 }}>
            <View style={styles.progressLabelRow}>
              <Text style={{ fontFamily: FONTS.family.medium, fontSize: SIZES.font.xs, color: COLORS.textSecondary }}>Scheme completion</Text>
              <Text style={{ fontFamily: FONTS.family.bold, fontSize: SIZES.font.xs, color: COLORS.primary }}>{scheme.progress}%</Text>
            </View>
          ...
          <View style={styles.tagsRow}>
            {scheme.tags.map(tag => (
              <View key={tag} style={[styles.tag, { borderColor: COLORS.border }]}>
                <Text style={[styles.tagText, { fontFamily: FONTS.family.semiBold, fontSize: SIZES.font.xxs, color: COLORS.textSecondary }]}>{tag}</Text>
              </View>
            ))}
          </View>
        )}
          ...
          <>
            ...
            <View style={styles.footer}>
              <View>
                ...
                {scheme.nextDueDate && <Text style={{ fontFamily: FONTS.family.regular, fontSize: SIZES.font.xxs, color: COLORS.textTertiary, marginTop: 2 }}>Next due: {scheme.nextDueDate}</Text>}
              </View>
              <TouchableOpacity style={[styles.navBtn, { backgroundColor: COLORS.backgroundGold, borderColor: COLORS.secondaryLighter }]} onPress={() => onPress?.(scheme.id)} activeOpacity={0.75}>
            ...
            {isExpired ? (
              <>
                <TouchableOpacity style={[styles.btn, styles.btnDisabled]} disabled activeOpacity={1}>
                  ...
                  <Text style={[styles.btnText, { color: COLORS.textTertiary, fontFamily: FONTS.family.semiBold, fontSize: SIZES.font.sm }]}>Enrolment closed</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.btn, styles.btnOutline, { borderColor: COLORS.border }]} onPress={() => onRedeem?.(scheme.id)} activeOpacity={0.75}>
                  ...
                  <Text style={[styles.btnText, { color: COLORS.textSecondary, fontFamily: FONTS.family.semiBold, fontSize: SIZES.font.sm }]}>Redeem</Text>
                </TouchableOpacity>
              </>
            ) : (
              <>
                <TouchableOpacity style={[styles.btn, styles.btnOutline, { borderColor: COLORS.border }]} onPress={() => onDetails?.(scheme.id)} activeOpacity={0.75}>
                  ...
                  <Text style={[styles.btnText, { color: COLORS.textSecondary, fontFamily: FONTS.family.semiBold, fontSize: SIZES.font.sm }]}>Details</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.btn, styles.btnGold, { backgroundColor: COLORS.secondary }]} onPress={() => onBuy?.(scheme.id)} activeOpacity={0.8}>
                  ...
                  <Text style={[styles.btnText, { color: COLORS.white, fontFamily: FONTS.family.semiBold, fontSize: SIZES.font.sm }]}>Start SIP</Text>
                </TouchableOpacity>
              </>
            )}
          </View>
...
function HorizontalCard({ scheme, loading, onPress, onBuy, onDetails }) {
  const { COLORS, FONTS, SIZES } = useTheme();
  const isExpired = scheme.status === 'expired';
  const statusCfg = STATUS_CONFIG[scheme.status];
    ...
    return (
      <View style={[styles.hCard, { backgroundColor: COLORS.card }]}>
        ...
        <View style={{ flex: 1, padding: 12, gap: 8 }}>
          <Shimmer width="75%" height={12} /><Shimmer width="55%" height={10} />
          <View style={{ flexDirection: 'row', gap: 8 }}><Shimmer width={36} height={28} borderRadius={6} /><Shimmer width={36} height={28} borderRadius={6} /></View>
          <View style={{ flexDirection: 'row', gap: 6 }}><Shimmer width="45%" height={32} borderRadius={10} /><Shimmer width="45%" height={32} borderRadius={10} /></View>
        </View>
      </View>
    );
  }
  ...
  return (
    <Pressable style={[styles.hCard, { backgroundColor: COLORS.card }]} onPress={() => onPress?.(scheme.id)} android_ripple={{ color: COLORS.orangeOpacity10 }}>
      ...
      <View style={styles.hBody}>
        <View>
          <Text style={[styles.hName, { fontFamily: FONTS.family.bold, fontSize: SIZES.font.sm, color: COLORS.textPrimary }]} numberOfLines={1}>{scheme.name}</Text>
          <Text style={[styles.hDesc, { fontFamily: FONTS.family.regular, fontSize: SIZES.font.xs, color: COLORS.textSecondary }]} numberOfLines={2}>{scheme.description}</Text>
          <View style={styles.hStats}>
            <View><Text style={{ fontFamily: FONTS.family.bold, fontSize: SIZES.font.sm, color: COLORS.secondaryDark }}>{scheme.purity}</Text><Text style={{ fontFamily: FONTS.family.regular, fontSize: SIZES.font.xxs, color: COLORS.textTertiary }}>Purity</Text></View>
            <View><Text style={{ fontFamily: FONTS.family.bold, fontSize: SIZES.font.sm, color: COLORS.successDark }}>{scheme.returns}</Text><Text style={{ fontFamily: FONTS.family.regular, fontSize: SIZES.font.xxs, color: COLORS.textTertiary }}>Returns</Text></View>
            <View><Text style={{ fontFamily: FONTS.family.bold, fontSize: SIZES.font.sm, color: COLORS.textPrimary }}>₹{scheme.minAmount.toLocaleString('en-IN')}</Text><Text style={{ fontFamily: FONTS.family.regular, fontSize: SIZES.font.xxs, color: COLORS.textTertiary }}>Min</Text></View>
          </View>
        </View>
        <View style={styles.hActions}>
          <TouchableOpacity style={[styles.hBtn, { backgroundColor: COLORS.gray100 }]} onPress={() => onDetails?.(scheme.id)} activeOpacity={0.75}>
            <Text style={{ fontFamily: FONTS.family.semiBold, fontSize: SIZES.font.xxs, color: COLORS.textSecondary }}>Details</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.hBtn, { backgroundColor: isExpired ? COLORS.gray200 : COLORS.secondary }]} onPress={() => !isExpired && onBuy?.(scheme.id)} disabled={isExpired} activeOpacity={0.8}>
            <Text style={{ fontFamily: FONTS.family.semiBold, fontSize: SIZES.font.xxs, color: isExpired ? COLORS.textTertiary : COLORS.white }}>{isExpired ? 'Closed' : 'Join'}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Pressable>
...
function MiniCard({ scheme, loading, onPress, onBuy }) {
  const { COLORS, FONTS, SIZES } = useTheme();
  const isExpired = scheme.status === 'expired';
  const statusCfg = STATUS_CONFIG[scheme.status];
    ...
    return (
      <View style={[styles.miniCard, { backgroundColor: COLORS.card }]}>
        ...
        <View style={{ padding: 10, gap: 6 }}>
          <Shimmer width="80%" height={11} /><Shimmer width="55%" height={16} />
          <Shimmer width="40%" height={9} /><Shimmer width="100%" height={32} borderRadius={10} style={{ marginTop: 4 }} />
        </View>
      </View>
    );
  }
  ...
  return (
    <Pressable style={[styles.miniCard, { backgroundColor: COLORS.card }]} onPress={() => onPress?.(scheme.id)} android_ripple={{ color: COLORS.orangeOpacity10 }}>
      <SchemeImage image={scheme.image} height={80} gradientColors={[COLORS.primaryPale, COLORS.secondaryLighter]}>
        <View style={{ padding: 6 }}>
          <View style={[styles.badge, { backgroundColor: statusCfg.bg }]}>
            <Text style={[styles.badgeText, { fontFamily: FONTS.family.bold, fontSize: 8, color: statusCfg.color }]}>{statusCfg.label}</Text>
          </View>
        </View>
      ...
      <View style={styles.miniBody}>
        <Text style={[styles.miniName, { fontFamily: FONTS.family.bold, fontSize: SIZES.font.sm, color: COLORS.textPrimary }]} numberOfLines={1}>{scheme.name}</Text>
        <Text style={{ fontFamily: FONTS.family.extraBold, fontSize: SIZES.font.xl, color: COLORS.secondaryDark }}>{scheme.returns}</Text>
        <Text style={{ fontFamily: FONTS.family.semiBold, fontSize: SIZES.font.xxs, color: COLORS.textTertiary, textTransform: 'uppercase', letterSpacing: 0.3 }}>Returns · {scheme.purity}</Text>
        <TouchableOpacity style={[styles.miniBtn, { backgroundColor: isExpired ? COLORS.gray200 : COLORS.secondary }]} onPress={() => !isExpired && onBuy?.(scheme.id)} disabled={isExpired} activeOpacity={0.8}>
          <Text style={{ fontFamily: FONTS.family.bold, fontSize: SIZES.font.xs, color: isExpired ? COLORS.textTertiary : COLORS.white }}>{isExpired ? 'Closed' : 'Invest →'}</Text>
        </TouchableOpacity>
      </View>
    </Pressable>
  );
...
export default function AppSchemeCard(props) {
...
const styles = StyleSheet.create({
  card:             { borderRadius: 18, overflow: 'hidden', borderWidth: 0.5, borderColor: '#E5E5E5' },
  body:             { padding: 14 },
